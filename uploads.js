...
export class Uploads {
  ...
  async getDurationOfUpload(fileName, bucketName) {
    if (!(fileName in FILESIZE_BYTES) || !(fileName in FILESIZE_MIB)) {
      const errorMessage = `Invalid File Name: '${fileName}'. File names must be any of "2mib.txt", "64mib.txt" or "256mib.txt"`;
      throw new Error(errorMessage);
    }
    if (!(bucketName in REGIONS_MAP)) {
      const errorMessage = `Invalid Bucket Name: '${bucketName}'. Bucket must be a supported Google Cloud Storage Region Name. View https://cloud.google.com/storage/docs/locations for more information.`;
      throw new Error(errorMessage);
    }
  ...
  async benchmarkSingleUpload(fileName, bucketName) {
    ...
    const fileSizeBytes = FILESIZE_BYTES[fileName] || fileName;
    const fileSizeMiB = FILESIZE_MIB[fileName] || fileName;
    const location = REGIONS_MAP[bucketName] || bucketName;
