...
describe('uploads', () => {
  ...
  describe('getSignedURL', async () => {
    it('should form the source of the signed URL correctly', async () => {
      const spyAxiosGetMethod = sinon
        .stub(axios, 'get')
        .callsFake(fakeAxiosSuccess);
      ...
      assert.deepStrictEqual(spyAxiosGetMethod.args[0], [
        `http://localhost:3000/${bucketName}/${fileName}`,
      ]);
    });
    ...
    it('should throw an error when axios request fails', async () => {
      ...
      await assert.rejects(uploads.getSignedURL(fileName, bucketName), {
        message: ERR_MSG_FAILED_TO_REACH_SERVER,
      });
    });
  });
  ...
  describe('getDurationOfUpload', () => {
    it('should throw Error if bucketName is invalid', async () => {
      ...
      await assert.rejects(uploads.getDurationOfUpload(fileName, bucketName), {
  ...
  describe('benchmarkSingleUpload', () => {
    it('should throw Error if fileName is invalid', async () => {
      ...
      await assert.rejects(
        uploads.benchmarkSingleUpload(fileName, bucketName),
        { message: ERR_MSG_INVALID_FILE }
      );
    });
