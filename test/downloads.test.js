...
import {
  ERR_MSG_INVALID_BUCKET,
  ERR_MSG_INVALID_FILE,
  fakePerformanceNow,
} from './common.test.js';
...
async function fakeAxiosGet(URL) {
  ...
  const regexURL =
    'https://storage.googleapis.com/([a-zA-Z]+(-[a-zA-Z]+)+)[0-9]+/[0-9]+([a-zA-Z]+(.[a-zA-Z]+)+)';
...
describe('downloads', () => {
  ...
  describe('getDurationInSeconds', () => {
    it('should throw Error if bucketName is invalid', async () => {
      ...
      await assert.rejects(
        downloads.getDurationInSeconds(fileName, bucketName),
        { message: ERR_MSG_INVALID_BUCKET }
      );
    });
    ...
    it('should return -0.001 if GET request fails', async () => {
      sinon
        .stub(downloads, 'getDurationOfGetRequest')
        .callsFake(fakeGetDurationOfGetRequest);
    ...
    it('should build URL correctly', async () => {
      ...
      assert.deepStrictEqual(calledWith[calledWith.length - 1], [
        'https://storage.googleapis.com/gcsrbpa-us-west1/2mib.txt',
      ]);
    });
  });
  ...
  describe('benchmarkSingleDownload', () => {
    it('should return an Array of an Object on success', async () => {
      const fileName = '2mib.txt';
      const bucketName = 'us-west1';
      const result = await downloads.benchmarkSingleDownload(
        fileName,
        bucketName
      );
    ...
    it('should throw Error if fileName is invalid', async () => {
      ...
      await assert.rejects(
        downloads.benchmarkSingleDownload(fileName, bucketName),
        { message: ERR_MSG_INVALID_FILE }
      );
    });
